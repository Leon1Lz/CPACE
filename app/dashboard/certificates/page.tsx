...
export default function CertificatesPage() {
  ...
  useEffect(() => {
    fetch("/api/certificates?limit=100").then(r => r.json()).then(data => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Total Issued", value: certificates.length, gradient: "from-violet-500 to-purple-600" },
          { label: "Valid", value: certificates.filter(c => c.isValid).length, gradient: "from-emerald-500 to-teal-600" },
          { label: "Courses", value: new Set(certificates.map(c => c.courseId)).size, gradient: "from-blue-500 to-cyan-600" },
        ].map((s, i) => (
